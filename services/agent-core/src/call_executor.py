...
def call_agent_runner_api(base_url: str, function_name: str, args: dict):
    ...
    if function_name == "createOperationOperationsPost":
        ...
        r = session.post(f"{base_url}/operations", json=args, headers=headers, timeout=TOTAL_TIMEOUT)
    elif function_name == "executeActionV1AgentExecutePost":
        # Ejecución POST /v1/agent/execute
        r = session.post(f"{base_url}/v1/agent/execute", json=args, headers=headers, timeout=TOTAL_TIMEOUT)
    else:
        # Esto nunca debería pasar si Gemini usa el Tool correcto
        return {"error": f"Función no implementada o fuera del contrato: {function_name}", "request_id": request_id}
