    ...
    while response.function_calls:
        ...
        for fc in response.function_calls:
            ...
            try:
                ...
                tool_output_part = types.Part.from_function_response(name=name, response={"result": json.dumps(result)})
                tool_outputs.append(tool_output_part)
                print(f"  > Salida OK: {json.dumps(result)[:50]}...")
            except requests.exceptions.HTTPError as e:
                # Manejo de error HTTP (4xx/5xx)
                error_details = {"error": f"HTTP Error: {e.response.status_code}", "details": e.response.text[:100]}
                tool_output_part = types.Part.from_function_response(name=name, response={"error": json.dumps(error_details)})
                tool_outputs.append(tool_output_part)
                print(f"  > Error HTTP: {e.response.status_code}")
            except Exception as e:
                # Otros errores (conexión, timeout, etc.)
                error_details = {"error": f"Error de Ejecución: {type(e).__name__}", "details": str(e)[:100]}
                tool_output_part = types.Part.from_function_response(name=name, response={"error": json.dumps(error_details)})
                tool_outputs.append(tool_output_part)
        ...
        response = client.models.generate_content(
            model=model,
            contents=[prompt, response] + tool_outputs, # Agregar el historial de la función y su output
            config=types.GenerateContentConfig(tools=[tool])
        )

    # 5. Respuesta Final (Texto)
    print("\n✅ Respuesta Final del LLM:")
    print(getattr(response, 'text', str(response))) # Imprimir el resultado final del razonamiento
...
if __name__ == "__main__":
    ...
    run_agent_loop("Crea una nueva operación con la acción 'analyze_data' y el payload 'reporte_trimestral'.")
